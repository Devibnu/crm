...
const canComposeEmail = computed(() => ability.can('create', 'CrmInbox'))
const canManageEmail = computed(() => ability.can('update', 'CrmInbox'))
const emailAccessMessage = computed(() => {
  if (canComposeEmail.value || canManageEmail.value)
    return 'Anda dapat melihat inbox email. Aksi tulis hanya tampil sesuai kemampuan inbox pada peran ini.'

  return 'Mode baca aktif. Anda hanya dapat membaca email tanpa compose, reply, atau mengubah status mailbox.'
})
...
const handleComposeSend = async (payload: ComposeEmailPayload) => {
  ...
  try {
    ...
    composeError.value = error?.data?.message || 'Failed to send email.'
  }
  finally {
  ...
  >
    ...
    >
      <EmailLeftSidebarContent
        ...
        :can-compose-email="canComposeEmail"
    ...
    <EmailView
      ...
      :can-manage-email="canManageEmail"
      :can-compose-email="canComposeEmail"
    ...
    <VMain>
      ...
      >
        ...
        <div class="py-2 px-4 d-flex align-center d-flex gap-x-1">
          ...
          >
            ...
            <!-- Mark unread/read -->
            <IconBtn v-if="canManageEmail" @click="isAllMarkRead ? handleActionClick('unread') : handleActionClick('read') ">
              <VIcon
        ...
        >
          ...
          >
            ...
            <div class="email-actions d-none">
              <IconBtn v-if="canManageEmail" @click.stop="handleActionClick('trash', [email.id])">
                <VIcon
                  icon="tabler-trash"
                  size="22"
                />
              ...
              <IconBtn v-if="canManageEmail" @click.stop="handleActionClick('spam', [email.id])">
                <VIcon
      ...
      <ComposeDialog
        v-show="isComposeDialogVisible && canComposeEmail"
