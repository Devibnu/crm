...
const canCreateInvoices = computed(() => ability.can('create', 'BackofficeInvoice'))
const canUpdateInvoices = computed(() => ability.can('update', 'BackofficeInvoice'))
const canDeleteInvoices = computed(() => ability.can('delete', 'BackofficeInvoice'))
const invoiceAccessMessage = computed(() => {
  if (canCreateInvoices.value || canUpdateInvoices.value || canDeleteInvoices.value)
    return 'Anda dapat melihat invoice, tetapi beberapa aksi finansial tetap dibatasi sesuai peran.'

  return 'Mode baca aktif. Anda hanya dapat melihat daftar dan detail invoice tanpa membuat atau mengubah data.'
})
    ...
    <VCard id="invoice-list">
      ...
      >
        ...
        <template #item.actions="{ item }">
          ...
          <MoreBtn
            v-if="computedMoreList(item.id).length"
