...
import { db as authDb } from '@db/auth/db'
...
export const handlerAppsUsers = [
  ...
  // Update User Access
  http.put<PathParams>(('/api/apps/users/:id'), async ({ request, params }) => {
    const userId = Number(params.id)
    const payload = await request.json() as Partial<typeof db.users[0]>
    ...
    const authUser = authDb.users.find(user => user.email === currentUser.email)

    if (authUser) {
      authUser.role = currentUser.role
      authUser.modulePermissions = currentUser.modulePermissions
      authUser.abilityRules = buildAuthAbilityRules({
        role: authUser.role,
        modulePermissions: authUser.modulePermissions,
        existingRules: authUser.abilityRules,
      })
    }
