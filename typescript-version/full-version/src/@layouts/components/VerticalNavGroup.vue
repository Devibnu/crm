...
watch(
  () => route.path,
  () => {
    const isActive = isGroupRouteActive() || isNavGroupActive(props.item.children, router)
    ...
    >
      ...
      >
        ...
        <span
          v-show="!hideTitleAndBadge"
          key="arrow"
          class="nav-group-arrow d-inline-flex"
          @click.stop="toggleGroupOnly"
        >
          <Component
            :is="layoutConfig.app.iconRenderer || 'div'"
