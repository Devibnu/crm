...
const isPathString = (value: NavLinkProps['to']): value is string => typeof value === 'string' && value.startsWith('/')
...
export const getComputedNavLinkToProp = computed(() => (link: NavLink) => {
  ...
  if (link.to)
    props.to = typeof link.to === 'string' ? (isPathString(link.to) ? link.to : { name: link.to }) : link.to
...
export const isNavLinkActive = (link: NavLink, router: Router) => {
  const resolvedLink = link.to ? router.resolve(link.to) : null

  if (resolvedLink?.path === router.currentRoute.value.path)
    return true
