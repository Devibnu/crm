...
export const can = (action: string | undefined, subject: string | undefined) => {
  const userData = useCookie<{ role?: string } | null>('userData')
...
export const canNavigate = (to: RouteLocationNormalized) => {
  ...
  const ability = createMongoAbility(buildAuthAbilityRules({
  ...
  if (to.name === 'dashboard') {
    return ability.can('manage', 'Admin')
      || ability.can('read', 'CrmCustomers')
      || ability.can('read', 'CrmTickets')
      || ability.can('read', 'CrmInbox')
  }
