...
const translateActionTerm = (value: string) => {
  if (value === 'List' || value === 'Preview' || value === 'Edit' || value === 'Add')
    return t(value)

  return value
}

const translateSearchTitle = (value: string) => {
  return value
    .replace(/\bList\b/g, t('List'))
    .replace(/\bPreview\b/g, t('Preview'))
    .replace(/\bEdit\b/g, t('Edit'))
    .replace(/\bAdd\b/g, t('Add'))
}
...
<template>
  ...
  >
    ...
    >
      <span class="me-2">{{ t('common.search') }}</span>
  ...
  >
    ...
    <template #searchResult="{ item }">
      ...
      >
        ...
        <VListItemTitle>
          {{ translateSearchTitle(list.title) }}
        </VListItemTitle>
      </VListItem>
    </template>
  </LazyAppBarSearch>
