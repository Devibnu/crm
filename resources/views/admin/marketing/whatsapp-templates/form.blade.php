...
@section('content')
        ...
        <div class="wa-form-card">
            ...
            <form method="POST" action="{{ $template ? route('admin.marketing.whatsapp-templates.update', $template) : route('admin.marketing.whatsapp-templates.store') }}" class="wa-form-grid">
                ...
                <div class="wa-form-fields">
                    <label><span>Pilih template siap pakai</span><select id="preset-select"><option value="">Custom</option>@foreach($presets as $key => $preset)<option value="{{ $key }}" data-category="{{ $preset['category'] }}" data-body="{{ $preset['body'] }}">{{ $key }}</option>@endforeach</select></label>
                    <label><span>Nama Template</span><input id="template-name" name="name" value="{{ old('name', $template->name ?? '') }}" required></label>
                    <label><span>Safe Name otomatis untuk Meta</span><input id="safe-name" value="{{ old('safe_name', $template->safe_name ?? $template?->name ?? '') }}" readonly></label>
                    <label><span>Kategori</span><select id="category" name="category" required>@foreach(['UTILITY','MARKETING','AUTHENTICATION'] as $category)<option value="{{ $category }}" @selected(old('category', $template->category ?? 'UTILITY') === $category)>{{ $category }}</option>@endforeach</select></label>
    ...
    <script>
        ...
        const nameInput = document.getElementById('template-name'), safeInput = document.getElementById('safe-name'), category = document.getElementById('category'), body = document.getElementById('body'), header = document.getElementById('header'), footer = document.getElementById('footer');
        ...
        const update = () => {
            ...
            document.getElementById('readiness-score').textContent = reasons.length === 0 ? 'High Approval Chance' : (reasons.length <= 1 ? 'Medium' : 'Risky');
